...
from app.base.constants.BM_CONSTANTS import plot_zip_locations, plot_zip_download_location
...
def selectds(ds_source, ds_goal):
    return render_template('applications/pages/connecttods.html', ds_id=ds_source, ds_goal=ds_goal, segment='createmodel')
...
def uploadcsvds():
    if request.method == 'POST':
        ...
        if (ds_goal == current_app.config['PREDICTION_MODULE']):  # Prediction
        ...
        if (ds_goal == current_app.config['CLASSIFICATION_MODULE']):  # Classification
        ...
        if (ds_goal == current_app.config['ROBOTIC_MODULE']):  # Robotics
...
def creatingthemodel():
    try:
        if request.method == 'POST':
            ...
            ds_source = request.form.get('ds_source')
            ds_goal = request.form.get('ds_goal')
            loading_icon_path = os.path.join('static/images/', 'loading_icon.gif')
            progress_icon_path = os.path.join('static/images/', 'progress_icon_2.gif')
            if (ds_goal == current_app.config['PREDICTION_MODULE']):
    ...
    try:
        if request.method == 'POST':
            ...
            if (ds_goal == current_app.config['PREDICTION_MODULE']):
                ...
                # run model
                model_controller = run_prediction_model(root_path, data_file_path,featuresdvalues, predictionvalues, ds_source, ds_goal,
                                                        app.config['DEMO_KEY'])
            ...
            if (ds_goal == current_app.config['CLASSIFICATION_MODULE'] and ds_source == '11'):
                ...
                return classification_director.create_text_classification_model(location_details,  ds_goal, ds_source,
                                                                                is_local_data)
            ...
            if (ds_goal == current_app.config['CLASSIFICATION_MODULE']):
                ...
                return render_template('applications/pages/classification/modelstatus.html',
                                       accuracy=model_controller['accuracy'],
                                       confusion_matrix=model_controller['confusion_matrix'],
                                       plot_image_path=model_controller['plot_image_path'],
                                       Mean_Absolute_Error=model_controller['Mean_Absolute_Error'],
                                       Mean_Squared_Error=model_controller['Mean_Squared_Error'],
                                       Root_Mean_Squared_Error=model_controller['Root_Mean_Squared_Error'],
                                       segment='createmodel', page_url=page_url, page_embed=page_embed,
...
def runthemodel_():
    try:
        ...
        ds_goal = ModelProfile.query.with_entities(ModelProfile.ds_goal).first()[0]
        ...
        print(e)
        return render_template('page-501.html', error="blablabl", segment='error')
